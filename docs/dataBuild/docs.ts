...
// import {renderToMd} from "./embeddingsRenderer";
...
} from "@edgedb-site/build-tools/git";
...
export class DocsPipeline extends Pipeline {
              ...
              fs
                ...
                .then((data) => {
                  ...
                  return setCacheItem(
                    ...
                    buildIntersphinxInv({ name, version }, remappedRefs)
                  );
                }),
            ]);
  ...
  async processXMLFile(file: string, fileInfo: FileInfo) {
    ...
    // if (flags.embeddings) {
    //   const destDir = pathJoin(__dirname, "../.build-cache/docs/gpt");
    ...
    //         pathJoin(
    //           __dirname,
    //           `../.build-cache/docs/gpt/${fileInfo.relname}${i ? i : ""}.md`
    //         ),
