...
export default async function CloudPages({ params }: PageProps) {
  const { doc } = await getDocsDocument(["cloud", ...(params?.path ?? [])]);
...
export async function generateStaticParams() {
  const docsPaths = await getDocsPaths("cloud");
...
export async function generateMetadata({
  ...
  const title = await getDocTitle(["cloud", ...(params?.path ?? [])]);
