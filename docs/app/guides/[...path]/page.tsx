...
export default async function GuidesPages({ params }: PageProps) {
  const { doc } = await getDocsDocument(["guides", ...(params?.path ?? [])]);
...
export async function generateStaticParams() {
  const docsPaths = await getDocsPaths("guides");
...
export async function generateMetadata({
  ...
  const title = await getDocTitle(["guides", ...(params?.path ?? [])]);
