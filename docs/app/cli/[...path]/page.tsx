...
export default async function CLIPages({ params }: PageProps) {
  const { doc } = await getDocsDocument(["cli", ...(params?.path ?? [])]);
...
export async function generateStaticParams() {
  const docsPaths = await getDocsPaths("cli");
...
export async function generateMetadata({
  ...
  const title = await getDocTitle(["cli", ...(params?.path ?? [])]);
