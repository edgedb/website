...
export default async function GetStartedPages({ params }: PageProps) {
  const { doc } = await getDocsDocument(["intro", ...params.path]);
...
export async function generateStaticParams() {
  const docsPaths = await getDocsPaths("intro");
...
export async function generateMetadata({
  ...
  const title = await getDocTitle(["intro", ...(params?.path ?? [])]);
