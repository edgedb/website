...
export function useVisibleElement(
  ...
  useEffect(() => {
    ...
    // and calls callback with the element covering the most of the central 50%
    // of the page.
    function update() {
      let overlap = 0;
      ...
      const topBoundary = windowHeight / 4;
      const bottomBoundary = windowHeight * (3 / 4);
      const boundaryHeight = windowHeight / 2;
      ...
      for (const el of visibleElements) {
        ...
        const elOverlap = (overlapBottom - overlapTop) / boundaryHeight;
