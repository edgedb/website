...
export class BlogPipeline extends Pipeline {
  ...
  async extractMeta(node: XMLNode, isIndex: boolean) {
    ...
    let leadImgAltNodes = loadNodes("blog-lead-image-alt");
    let leadYTNodes = loadNodes("blog-lead-youtube");
    let localLinkNodes = loadNodes("blog-local-link");
    let draftNodes = loadNodes("blog-draft");
    let recommendationNodes = loadNodes("blog-recommendation");
    ...
    if (isIndex) {
      ...
      recommendations = recommendationNodes.map((node) => node.getText());
      ...
      leadImageAlt = leadImgAltNodes[0]?.getText() ?? null;

      if (leadYTNodes.length) {
        leadYT = leadYTNodes[0].attrs.ytid!;
      }
