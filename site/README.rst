...
The first step is done during building, in order to create markdown files for docs add ``--embeddings`` flag to the building script.
When markdown files are generated in ``.build-cache/docs/gpt`` run script ``yarn embeddings``.
