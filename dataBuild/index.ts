...
if (
  new Set(docVersionsConfig.versions.map((v) => v.id)).size !==
  docVersionsConfig.versions.length
) {
  throw new Error("Doc version ids must be unique");
}
...
const runner = new PipelineRunner({
  ...
  pipelines: [
    new MetaPipeline(),
    ...(flags.watch
      ? [new DocsPipeline("dev")]
      : docVersionsConfig.versions.map(({id}) => new DocsPipeline(id))),
