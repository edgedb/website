...
import "@/components/code/auto-grammars";
...
import {CellResult, CellResultError, CodeCellState} from "tutorial/types";
...
export default function CodeCell({
  ...
  const highlighter = useCallback(
    (code) => prism(code, languages.edgeql, "edgeql"),
    []
  );
  const dispatch = useDispatch();

  const updateCodeCell = useCallback(
    (code: string) => dispatch(actions.updateCodeCell(id, code)),
    [dispatch, id]
  );

  const runCodeCell = useCallback(
    () => dispatch(actions.runCells({pagePath, cellId: id})),
    [dispatch, pagePath, id]
  );

  const isEdited = edgeql !== initialEdgeql;

  let errorOverlay: JSX.Element | null = null;
  if (!resultOutdated && result?.kind === "error") {
        ...
        <div className={styles.cellBorder}>
          ...
          <div className={styles.lowerBorder}>
            <div className={styles.controls}>
              <div className={styles.run} onClick={runCodeCell}>
                <span>
                  <RunIcon />
                  Run
                </span>
...
const CodeCellOutput = memo(function CodeCellOutput({
  cellId,
  pagePath,
  result,
  resultOutdated,
  ...
  if (result && result.kind === "data") {
    if (STATUSES_WITH_OUTPUT.has(result.status)) {
      ...
      dataStatus = (
        <div className={styles.dataStatus}>{result.status}: OK</div>
      );
    }
  }

  const toggleMode = () => {
    dispatch(actions.toggleCodeCellMode({cellId}));
    if (dataInspector) {
      dispatch(actions.runCells({cellId, pagePath}));
    }
  };
  ...
  return (
    <div
      className={cn(styles.cell, {
        ...
        [styles.outdated]: resultOutdated,
      ...
      <div className={styles.inner}>
        {dataInspector ?? dataStatus ?? dataError ?? (
          <div className={styles.unevaled}>
            {errorInCell !== null ? (
              <>
                {`An error occurred in cell ${errorInCell.num}`}
                <a href={`#codecell${errorInCell.id}`}>Go to cell</a>
              </>
            ) : (
