...
export function getDocsIndexDef(docsVersion: string) {
  const docsIndexPath = require(`@/build-cache/docs/${docsVersion}/docsearch.jolrindex`);
  const {urlMapping} = getNavData(docsVersion);
