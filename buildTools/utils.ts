...
export function pythonCmd() {
  return process.env.VIRTUAL_ENV ? "python" : "./.venv/bin/python";
}
...
export async function copyAsset(
  ...
  logger.log("copying asset:", sourcePath, "->", targetPath);
...
export async function copyImage(
  ...
  const {hash, width, height, thumbnail} = await processImage(sourcePath);

  let darkHash: string | null = null;
  try {
    const {hash: dHash, width: dWidth, height: dHeight} = await processImage(
      darkSourcePath
    );
    ...
    if (e instanceof ImageDimensionError) {
      throw e;
    }
  }
  ...
  logger.log("copying image:", sourcePath, "->", targetPath);
  ...
  if (darkHash) {
    ...
    const darkTargetPath = pathJoin(
      "images",
      darkTargetName + sizes + fileExt
    );

    logger.log("copying image:", darkSourcePath, "->", darkTargetPath);

    await fs.copy(darkSourcePath, darkTargetPath, {overwrite: false});
  }
